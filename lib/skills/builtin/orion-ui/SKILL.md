...
- **Visual verification (required after output changes):** After executing a cell that creates or changes visual output, call `inspect_output` for every changed output. For Plotly, review both the rendered image and collision/overflow diagnostics. Repair visual problems, re-execute, and reinspect until the output is clean. If a layout constraint is genuinely unavoidable, explain it explicitly instead of silently accepting it.
